...
-- A debt settlement reduces the debt (debit system debt) and pays cash
-- (credit ordinary account).  Its reversal must do the exact opposite:
-- debit the ordinary account and credit the system debt account.
--
-- The prior implementation accidentally reused the settlement sides for
-- debt reversals.  Income and recovery branches already use their inverse
-- settlement sides and remain unchanged here.
...
begin
  ...
  if p_expected_kind='debt' then
    ...
    v_tx_id:=public.insert_financial_event_ledger_transaction(p_household_id,v_event_id,'debt_settlement_reversal',p_occurred_at,'Annulation règlement : '||trim(p_reason),p_amount,v_account_id,null,v_account_id,v_debt_id,p_notes);
  else
